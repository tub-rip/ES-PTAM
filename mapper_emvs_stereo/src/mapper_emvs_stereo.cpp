...
bool MapperEMVS::getPoseAt(std::shared_ptr<tf::Transformer> tf_, const ros::Time& t, std::string world_frame_id, std::string frame_id, geometry_utils::Transformation& T) {
  ...
  if (!tf_->canTransform(world_frame_id, frame_id, t, error_msg)) {
      ...
      tf_->lookupTransform(world_frame_id, frame_id, t, tr);
